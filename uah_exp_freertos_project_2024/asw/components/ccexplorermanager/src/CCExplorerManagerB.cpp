...

{
  ...
  CDTCHandler * pSGuidance_TC_Data = EDROOMPoolCDTCHandler.AllocData();
   ...
   //Send message 
   Guidance.send(SGuidance_TC,pSGuidance_TC_Data,&EDROOMPoolCDTCHandler); 
}



bool	CCExplorerManager::EDROOM_CTX_Top_0::Gj()
	...
	do
	{
		...
		switch(edroomCurrentTrans.localId)
		{
			...
			case (HandleTC):
				...
				//Evaluate Branch Guidance
				else if( GFwdToGuidance() )
				{
					//Send Asynchronous Message 
					FFwdGuidance();

					//Branch taken is HandleTC_Guidance
					edroomCurrentTrans.localId =
						HandleTC_Guidance;
